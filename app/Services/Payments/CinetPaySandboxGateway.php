...
class CinetPaySandboxGateway implements PaymentGateway
{
    ...
    public function createIntent(Reservation $reservation, array $context = []): PaymentAttempt
    {
        ...
        return PaymentAttempt::query()->create([
            ...
            'provider' => $this->name(),
            'provider_reference' => 'cinetpay-sandbox-'.Str::upper(Str::random(12)),
            'currency' => $reservation->currency,
            'amount' => $rawAmount,
            'status' => $isGuarantee ? 'authorized' : 'created',
            'idempotency_key' => $context['idempotency_key'] ?? Str::uuid()->toString(),
            'payload' => ['environment' => config('services.cinetpay.environment', 'sandbox'), 'mode' => 'sandbox', 'reservation_ref' => $reservation->reservation_ref, 'is_guarantee' => $isGuarantee],
        ]);
    }
